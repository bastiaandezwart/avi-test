...
import { Student, AviLevel, Classification, ErrorObservations } from '../types';
...
const OBSERVATION_OPTIONS: { key: keyof ErrorObservations; label: string }[] = [
  { key: 'spellingReading', label: 'Spellend lezen' },
  { key: 'skippedWords', label: 'Woord overgeslagen' },
  { key: 'mispronunciation', label: 'Verklanking' },
  { key: 'selfCorrections', label: 'Zelfcorrectie' },
];
...
export function TestScreenPage({ student, level, onSaveResult }: TestScreenPageProps) {
  ...
  const toggleObservation = (key: keyof ErrorObservations) => {
  ...
  const handleStop = () => {
    ...
    const classification = classify(
      errors, elapsed,
      config.goodMaxErrors, config.goodMaxSeconds,
      config.sufficientMaxErrors, config.sufficientMaxSeconds
    );
    const hasObservations = Object.values(observations).some(Boolean);
    const resultId = onSaveResult({
      ...
      observations: hasObservations ? observations : undefined,
    });
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-6 flex flex-col">
      <header className="bg-blue-600 text-white px-4 py-4 shadow-md flex items-center gap-3">
        <button
          onClick={() => { stopTimer(); navigate(`/student/${student.id}/test`); }}
      ...
      <div className="flex-1 px-4 py-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
          ...
          {!started ? (
            <button onClick={startTimer} className="mt-5 w-full bg-green-600 hover:bg-green-700 active:bg-green-800 text-white font-bold py-5 px-6 rounded-xl text-2xl shadow-md transition-colors min-h-[72px]">
              ▶ START
            </button>
          ) : timerRunning ? (
            <button onClick={() => stopTimer()} className="mt-5 w-full bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-white font-bold py-4 px-6 rounded-xl text-xl shadow transition-colors min-h-[64px]">
              ⏸ Pauzeer
            </button>
          ) : (
            <div className="mt-5">
              <button onClick={startTimer} className="w-full bg-green-600 text-white font-bold py-4 rounded-xl text-lg shadow transition-colors min-h-[56px]">
                ▶ Hervat
              </button>
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="flex items-center justify-center gap-4">
            <button onClick={handleRemoveError} disabled={errors === 0}
              ...
              aria-label="Fout verwijderen">−</button>
            <div className="text-6xl font-black text-gray-900 min-w-[80px] text-center tabular-nums">{errors}</div>
            <button onClick={handleAddError}
              ...
              aria-label="Fout toevoegen">+</button>
          </div>
        ...
        {/* Observations */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <p className="text-sm text-gray-500 font-medium mb-3">Leesgedrag (optioneel)</p>
          <div className="grid grid-cols-2 gap-2">
            {OBSERVATION_OPTIONS.map(({ key, label }) => {
              const active = !!observations[key];
              return (
                <button
                  key={key}
                  type="button"
                  onClick={() => toggleObservation(key)}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <button onClick={() => setShowText(!showText)}
            ...
            <svg className={`w-5 h-5 text-gray-400 transition-transform ${showText ? 'rotate-180' : ''}`}
